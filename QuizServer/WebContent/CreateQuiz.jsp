...
<html>
		...
		<script>
			...
			$(document).ready(function()
			{
				...
				$("#submit").click(function(e)
				{
					...
					$.post('servlet/CreateServlet?method=createQuiz&json=' + json, function(data)
					{
						if(data == "true")
							alert("Quiz created");
			...
			function checkPings()
			{
				...
				$.post('servlet/QuizServlet?method=getPing&teacher=' + username, function(data)
				{
					if(data.length > 5)
					{
			...
			function getGrades()
			{
				...
				$.post('servlet/QuizServlet?method=getTeacherGrades', function(data)
				{
					if(data)
					{
			...
			{			
				...
				//json=%7B'username':'michael.bayles','password':'bayles'%7D&method=isTeacher
				$.post('servlet/CreateServlet?method=getCourses', function(data){
	...
	<body>
		...
		<div id="wrapper">		
			<p id="logout" style="float:right"><a href="#" onclick="logout()">Logout</a></p>
			<form>
				...
				<table id="gradeTable" style="margin-top:100px;" border=1>
					<tbody>
						<tr>
							<th>Student ID</th>
							<th>Course</th>
